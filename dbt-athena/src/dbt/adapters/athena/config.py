        ...
        if provided_spark_properties:
            ...
            if spark_jars:
                ...
                jar_updated_list = list(
                    set([item.strip() for item in jar_list + def_jar_list if item.strip()])
                )
...
class LambdaSparkSessionConfig(SparkSessionConfig):
    ...
    def get_lambda_function_name(self) -> str:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    "lambda_function_name is required configuration for lambda submission method"
                )
    ...
    def get_spark_properties(self) -> Dict[str, str]:
        ...
        spark_encryption = self.config.get("spark_encryption", False)
        default_spark_properties: Dict[str, str] = dict(
            **cast(dict[str, str], LAMBDA_SPARK_PROPERTIES.get("default", {})),
            **(
                cast(dict[str, str], LAMBDA_SPARK_PROPERTIES.get(table_type, {}))
                if table_type.lower() in ["iceberg", "hudi", "delta_lake"]
                else {}
            ),
            **(
                cast(dict[str, str], LAMBDA_SPARK_PROPERTIES.get("spark_encryption", {}))
                if spark_encryption
                else {}
            ),
        )
        ...
        if provided_spark_properties:
            ...
            if spark_jar_pckgs:
                ...
                jar_pckgs_updated_list = list(
                    set(
                        [
                            item.strip()
                            for item in jar_pckgs_list + def_jar_pckgs_list
                            if item.strip()
                        ]
                    )
                )
